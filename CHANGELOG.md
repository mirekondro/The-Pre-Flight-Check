...
### Added
- **Go support** (`go.mod`): `go build ./...` · `golangci-lint` → `go vet` ·
  `go test ./...` · `govulncheck`.
- **Rust support** (`Cargo.toml`): `cargo check` · `cargo clippy` ·
  `cargo test` · `cargo audit`. Optional tools are gated on presence.
- **Config layer** — a `[tool.pre-flight-check]` table in `pyproject.toml` or a
  standalone `.pre-flight-check.toml`: override a gate's command, add a gate, or
  `disable` one. `doctor` shows the active config source. Loaded via `tomllib`
  (Python 3.11+); older engines ignore it, staying zero-dep and 3.8-compatible.
- **Engine unit-test suite** (`tests/`, 51 pytest cases) + a CI `engine-tests`
  job covering detection, stage builders, `resolve_exe`, config, and filtering.
...
[Unreleased]: https://github.com/mirekondro/The-Pre-Flight-Check/compare/v1.4.0...HEAD
[1.4.0]: https://github.com/mirekondro/The-Pre-Flight-Check/compare/v1.3.0...v1.4.0
